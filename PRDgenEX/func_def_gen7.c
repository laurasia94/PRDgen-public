...
///	char PROJ[500];
	...
	double zz,phi;
	zz=dir[2];
	phi=asin(dir[1]/sqrt(1-zz*zz));
	phi/=(0.5*3.14159265358);
...
// generating structures
		...
		func_def_position_model06(pene, ndef[0], pdf_possia, pdf_posvac, occ_sia, occ_vac, limit_sia, limit_vac);
...
int func_def_position_model06(double pene, int ndef, double pdf_possia[][NPOS], double pdf_posvac[][NPOS], int occ_sia[][SB][SC][NREF0], int occ_vac[][SB][SC][NREF0], int limit_sia[][2], int limit_vac[][2])
{
	...
	int a1,b1,c1;
	int ncsize;
	double PNEIG_SIA=1.0-func_pdf_sia_cluster(pene,1);// cluster probability, SIA;
	double PNEIG_VAC=1.0-func_pdf_vac_cluster(pene,1);// vac
	int ipos_sia[ndef][4];


	int nn;
	//double rcut_vs=sqrt(2.0)+0.05;	// cutoff distance of vacancy-SIA distance in a fractional coordinate
        double rcut_vs=1.0+0.05;	// cutoff distance of vacancy-SIA distance in a fractional coordinate
					// this, sqrt(2.0)+0.05, corresponds to including 1st/2nd/3rd neighbors
	int n_neig_sia=3;
	int n_neig_vac=2;
        // for SIA
        j=0;
		...
		{	
			...
			do
			{
				...
				if(a0<0 || b0<0 || c0<0 || a0>limit_sia[0][1] || b0>limit_sia[1][1] || c0>limit_sia[2][1] || a0>=SA || b0>=SB || c0>=SC)	
				{  printf("errro2\n");  exit(1);  }
		...
		else 
		{
			...
			do
			{
				ncsize=(int)(2+(func_sia_cluster_max(pene)-2)*func_rand1());	// 2-35
				p2=func_rand1();
				if(p2<(func_pdf_sia_cluster(pene,ncsize)/func_pdf_sia_cluster(pene,2)))
				{
					jud2=1;
				}
			}while(jud2==0);
									
			// find a cluster center
			jud2=0;
                        do
                        {
                                a0=limit_sia[0][0]+(int)((limit_sia[0][1]-limit_sia[0][0]+1)*func_rand1());
			...
			for(s=1;s<ncsize;s++)
			{
				// decide the neighboring distance
				jud2=0;
                               	do
                               	{
					nn=rand()%n_neig_sia;
					p3=func_rand1();
					if(p3<func_neig_prob_sia(nn))	jud2++;
				}while(jud2==0);
				...
				if(nn==0)	// 1st neig
				{
					...
					do
					{
						k=(i+1)%2;	// changing the type
						if(i==0)	// if the refernce is corner
						{
							a1=a0-rand()%2;
							b1=b0-rand()%2;
							c1=c0-rand()%2;
						}
						else	// if the reference is center
						{
							a1=a0+rand()%2;
                                                        b1=b0+rand()%2;
                                                        c1=c0+rand()%2;
						...
						jud2++;
						if(occ_sia[a1][b1][c1][k]==0)	
						{
							occ_sia[a1][b1][c1][k]+=1;
			                            	ipos_sia[j][0]=a1;
                                                      	ipos_sia[j][1]=b1;
                                                      	ipos_sia[j][2]=c1;
                                                      	ipos_sia[j][3]=k;
						...
						if(jud2>100)	{  printf("error: too many loop1\n");  exit(1);  }
					}while(jud2>=0);
				}
				else if(nn==1)	// 2nd neighbor
				{
					jud2=0;
                               	 	do
                               	 	{
						...
						if(i1==0)	a1=a0+1-2*i2;
						else if(i1==1)       b1=b0+1-2*i2;
						else if(i1==2)       c1=c0+1-2*i2;
						else	{  printf("error in i\n");  exit(1); }
						...
						i3=rand()%2;
                                                ...
                                                if(i1==0)  {  b1=b0+1-2*i2;  c1=c0+1-2*i3;  }
                                                else if(i1==1)  {  c1=c0+1-2*i2;  a1=a0+1-2*i3;  }
                                                else if(i1==2)  {  a1=a0+1-2*i2;  b1=b0+1-2*i3;  }
                                                else    {  printf("error in i\n");  exit(1); }
	...
	// for vacancy
		...
		p1=func_rand1();
                		...
                		if(occ_vac[a0][b0][c0][i]==0)
                		{
                        		...
                        		if(dtem1<pdf_posvac[0][a0])
                        		{
                                		...
                                		if(dtem1<pdf_posvac[1][b0])
                                		{
                                        		...
                                        		{
								jud3=func_check_neighbor(ndef, ipos_sia, a0, b0, c0, i, rcut_vs);
								if(jud3==0)
								{
			...
			// determine the cluster size
                        ...
                        do
                        {
				ncsize=(int)(2+(func_vac_cluster_max(pene)-2)*func_rand1());    // 2-35
                                p2=func_rand1();
                                if(p2<(func_pdf_vac_cluster(pene,ncsize)/func_pdf_vac_cluster(pene,2)))
                                {
                                        jud2=1;
                                }
                        }while(jud2==0);

			// find the defect center
                        jud2=0;
                        do
                        {
                                a0=limit_vac[0][0]+(int)((limit_vac[0][1]-limit_vac[0][0]+1)*func_rand1());
				...
				// decide the neighboring distance
                                ...
                                do
                                {
                                        nn=rand()%n_neig_vac;
                                        p3=func_rand1();
                                        if(p3<func_neig_prob_vac(nn))   jud2++;
                                }while(jud2==0);
                                ...
                                if(nn==0)       // 1st neig
                                {
                                        ...
                                        do
                                        {
                                                ...
                                                jud2++;
                                                if(occ_vac[a1][b1][c1][k]==0)
                                                {
							jud3=func_check_neighbor(ndef, ipos_sia, a1, b1, c1, k, rcut_vs);
							if(jud3==0)
							{
                                                        	occ_vac[a1][b1][c1][k]+=1;
                                                        	j++;
                                                        	jud2=-100;
							}
...
double func_cluster_prob_sia(int i)
{
	double x=i-2.0;
	double y;
	y=0.82*exp(-x/0.83624)+0.18*exp(-x/7.50491);
...
double func_cluster_prob_vac(int i)
{
        double x=i-2.0;
        double y;
        y=0.82*exp(-x/0.49656)+0.18*exp(-x/1.44114);
...
{
        double prob[3]={1.0, 0.1323, 0.3040};

	if(i<0 || i>2)  {  printf("error in sia\n");  exit(1);  }
...
double func_neig_prob_vac(int i)
{
        ...
        if(i<0 || i>1)  {  printf("error in sia\n");  exit(1);  }
...
{
        double val1=ene1*1.1119+ene1*ene1*0.0199+4.0;
        return (int)val1;
}
...
{
        int nc1=nc0-1;
        if(nc1<0)       {  printf("error in nc1\n");  exit(1);  }

        double q0=1.41024535;
        double q1=-0.247107455;
        double w0=0.44657689;
        double b10=2.12588361;
        double b11=-0.289383920;
        double b20=1.27602796;
        double b21=-0.339341207;


        double r1=exp(-(b10+b11*log(ene1+1.0)));
        double r2=exp(-(b20+b21*log(ene1+1.0)));

        double val0,val1;
        val0=(q0+q1*log(ene1+1.0));
        val0=1.0/(1.0+exp(-val0));
        if(nc1==0)
        {
                val1=val0;
        }
        ...
        {
                val1=w0*(1.0-r1)*pow(r1,nc1-1)/(1.0-pow(r1,99))+(1.0-w0)*(1.0-r2)*pow(r2,nc1-1)/(1.0-pow(r2,99));
                val1*=(1.0-val0);
        }
...
{
        //double val1=5.5124*pow(ene1,0.2803);
        double val1=0.4755*ene1+4.7442;
        return (int)val1;
}
...
double func_pdf_vac_cluster(double ene1, int nc0)
{
        ...
        double q0=0.918816872;
        double q1=0.528760955;
        double w0=0.57415980;
        double b10=1.82183358;
        double b11=0.0519812343;
        double b20=1.15420056;
        double b21=-0.0177323171;
...
int func_check_neighbor(int ndef, int ipos_sia[][4], int a0, int b0, int c0, int s0, double rcut1f)
{
	...
	for(i=0;i<ndef;i++)
	{
		if(s0==ipos_sia[i][3])
		{
			dist1=sqrt(pow(ipos_sia[i][0]-a0,2.0)+pow(ipos_sia[i][1]-b0,2.0)+pow(ipos_sia[i][2]-c0,2.0));
		}
		else if(s0==0 && ipos_sia[i][3]==1)
		{
			dist1=sqrt(pow(ipos_sia[i][0]+0.5-a0,2.0)+pow(ipos_sia[i][1]+0.5-b0,2.0)+pow(ipos_sia[i][2]+0.5-c0,2.0));
		}
		else if(s0==1 && ipos_sia[i][3]==0)
                {
                        dist1=sqrt(pow(ipos_sia[i][0]-(a0+0.5),2.0)+pow(ipos_sia[i][1]-(b0+0.5),2.0)+pow(ipos_sia[i][2]-(c0+0.5),2.0));
                }
		else {  printf("error\n");  exit(1);  }
